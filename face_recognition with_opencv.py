...
for i in bilinen_yuzler:
    pic = face_recognition.load_image_file(i)  # Resmi yükle
    kodlamalar = face_recognition.face_encodings(pic)  # Encoding al
...
while True:
    ...
    rbg_kare = cv.cvtColor(kare, cv.COLOR_BGR2RGB)  # BGR -> RGB dönüşümü

    yuz_konumlari = face_recognition.face_locations(rbg_kare)  # Yüz konumlarını bul
    yuz_kodlari = face_recognition.face_encodings(rbg_kare, yuz_konumlari)  # Encoding hesapla
    #endonin denilen
    # 📌 Görünen her yüz için işlem yap
    for (ust, sag, alt, sol), yuz_kodlama in zip(yuz_konumlari, yuz_kodlari):
